    ...
    def search_memories(
        ...
        Args:
            ...
            memory_types: Types of memory to search ('short_term', 'long_term', or both).
                         If None, searches both types (default behavior for backward compatibility)
        """
        search_service = None
        try:
            logger.debug(
                f"Starting memory search for query '{query}' in user_id '{user_id}', assistant_id '{assistant_id}', session_id '{session_id}' with category_filter={category_filter}, memory_types={memory_types}"
            )
            ...
            results = search_service.search_memories(
                query, user_id, assistant_id, session_id, category_filter, limit, memory_types
            )
