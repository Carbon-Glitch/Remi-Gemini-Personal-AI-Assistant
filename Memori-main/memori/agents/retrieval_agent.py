    ...
    def _plan_search_with_fallback_parsing(self, query: str) -> MemorySearchQuery:
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Initial JSON parse failed for search planning, attempting repair...")
                
                # Second attempt: repair common errors
                try:
                    repaired_json = self._repair_json(response_text)
                    parsed_data = json.loads(repaired_json)
                    logger.info(f"Successfully repaired and parsed JSON for search planning")
                except json.JSONDecodeError as e2:
                    json_errors.append(f"After repair error: {e2}")
                    logger.warning(f"JSON repair failed for search planning: {e2}")
                    ...
                    try:
                        ...
                        if first_brace != -1 and last_brace != -1 and last_brace > first_brace:
                            ...
                            logger.info(f"Successfully extracted and parsed JSON for search planning")
                    except (json.JSONDecodeError, ValueError) as e3:
                        json_errors.append(f"After extraction error: {e3}")
                        logger.error(f"All JSON parsing attempts failed for search planning")
                        logger.debug(f"Raw response: {response_text[:500]}...")  # Log first 500 chars
                        logger.debug(f"All errors: {'; '.join(json_errors)}")
                        return self._create_fallback_query(query)
            
            if parsed_data is None:
                logger.error(f"Failed to parse JSON for search planning after all attempts")
                return self._create_fallback_query(query)
