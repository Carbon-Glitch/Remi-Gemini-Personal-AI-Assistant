    ...
    def _plan_search_with_fallback_parsing(self, query: str) -> MemorySearchQuery:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    "Initial JSON parse failed for search planning, attempting repair..."
                )
                ...
                try:
                    ...
                    logger.info("Successfully repaired and parsed JSON for search planning")
                    ...
                    try:
                        ...
                        if first_brace != -1 and last_brace != -1 and last_brace > first_brace:
                            ...
                            logger.info("Successfully extracted and parsed JSON for search planning")
                    except (json.JSONDecodeError, ValueError) as e3:
                        json_errors.append(f"After extraction error: {e3}")
                        logger.error(
                            "All JSON parsing attempts failed for search planning"
                        )
            ...
            if parsed_data is None:
                logger.error(
                    "Failed to parse JSON for search planning after all attempts"
                )
                return self._create_fallback_query(query)
