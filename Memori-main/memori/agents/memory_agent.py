...
"""
    ...
    async def _process_with_fallback_parsing(
        ...
        """
        try:
            # Enhanced system prompt for JSON output - optimized for Gemini
            json_system_prompt = (
                system_prompt
                + "\n\n=== CRITICAL JSON OUTPUT REQUIREMENTS ===\n"
                + "You MUST respond with ONLY a valid JSON object. Follow these rules:\n"
                + "1. Output ONLY the JSON object, no explanations, no markdown, no code blocks\n"
                + "2. Use double quotes for all strings and keys (never single quotes)\n"
                + "3. Do NOT include trailing commas\n"
                + "4. Do NOT include comments (// or /* */)\n"
                + "5. All boolean values must be lowercase: true, false\n"
                + "6. All null values must be lowercase: null\n"
                + "7. Start with { and end with } immediately, no extra text\n"
            ...
            try:
                ...
                json_errors.append(f"Initial parse error: {e}")
                logger.debug(f"Initial JSON parse failed for {chat_id}, attempting repair...")

                # Second attempt: repair common errors
                try:
                    repaired_json = self._repair_json(response_text)
                    parsed_data = json.loads(repaired_json)
                    logger.info(f"Successfully repaired and parsed JSON for {chat_id}")
                except json.JSONDecodeError as e2:
                    json_errors.append(f"After repair error: {e2}")
                    logger.warning(f"JSON repair failed for {chat_id}: {e2}")

                    # Third attempt: try to extract JSON from text
                    try:
                        ...
                        if first_brace != -1 and last_brace != -1 and last_brace > first_brace:
                            ...
                            repaired_extracted = self._repair_json(extracted_json)
                            parsed_data = json.loads(repaired_extracted)
                            logger.info(f"Successfully extracted and parsed JSON for {chat_id}")
                    except (json.JSONDecodeError, ValueError) as e3:
                        json_errors.append(f"After extraction error: {e3}")
                        logger.error(f"All JSON parsing attempts failed for {chat_id}")
                        logger.debug(f"Raw response: {response_text[:500]}...")  # Log first 500 chars
                        logger.debug(f"All errors: {'; '.join(json_errors)}")
                        return self._create_empty_long_term_memory(
                            "default", f"JSON parsing failed after all attempts: {'; '.join(json_errors)}"
                        )

            if parsed_data is None:
                logger.error(f"Failed to parse JSON for {chat_id} after all attempts")
                return self._create_empty_long_term_memory(
                    "default", f"JSON parsing failed: {'; '.join(json_errors)}"
                )
  ...
  "promotion_eligible": "boolean - Should be promoted to short-term"
    ...
    def _repair_json(self, json_str: str) -> str:
        ...
        try:
            ...
            cleaned_lines = []
            for line in lines:
                # Only remove comments if they're not inside a string
                # Simple heuristic: if there's an even number of quotes before //, it's outside a string
            ...
            # Use regex but be careful not to break valid JSON in strings
            # This regex looks for comma followed by optional whitespace and closing brace/bracket
            json_str = re.sub(r",(\s*[}\]])", r"\1", json_str)

            return json_str.strip()

        except Exception as e:
            logger.debug(f"JSON repair failed: {e}, returning original string")
            return json_str
