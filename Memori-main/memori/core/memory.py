...
class Memori:
        ...
        try:
            ...
            if hasattr(self, "_in_context_retrieval") and self._in_context_retrieval:
                ...
                results = self.db_manager.search_memories(
                    ...
                    limit=5,
                    memory_types=["short_term", "long_term"],  # 明确指定搜索短期和长期记忆
                )
                logger.debug(
            ...
            try:
                # 明确指定搜索短期和长期记忆，确保能从local_memory中读取长期记忆
                results = self.db_manager.search_memories(
                    query=user_input,
                    user_id=self.user_id,
                    assistant_id=self.assistant_id,
    ...
    def record_conversation(
        ...
        try:
            ...
            # Update conversation manager session history
            if response_text:
                # Ensure model info is in metadata for session history
                response_metadata = (metadata or {}).copy()
                if "model" not in response_metadata:
        ...
        try:
            ...
            if remaining_limit > 0:
                ...
                if self.search_engine:
                    ...
                    # 明确指定搜索短期和长期记忆，确保能从 local_memory 中读取所有记忆
